...
export const HotelCard: React.FC<HotelCardProps> = ({ hotel, searchContext }) => {
  ...
  return (
    ...
    >
      ...
      <div className="relative w-full aspect-[20/19] rounded-2xl overflow-hidden bg-slate-100 shadow-2xs">
        <img
        ...
        {photosList.length > 1 && (
          <>
            <button
              ...
              className="absolute left-2.5 top-1/2 -translate-y-1/2 w-7 h-7 rounded-full bg-white/90 hover:bg-white text-slate-800 shadow-md flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-200"
            >
            ...
            <button
              ...
              className="absolute right-2.5 top-1/2 -translate-y-1/2 w-7 h-7 rounded-full bg-white/90 hover:bg-white text-slate-800 shadow-md flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-200"
            >
        ...
        <div className="absolute top-3 left-3 flex items-center gap-1.5">
          {isGuestFavorite && (
            <span className="px-2.5 py-1 bg-white/95 backdrop-blur-md rounded-full text-[11px] font-bold text-slate-900 shadow-sm flex items-center gap-1 border border-slate-200/50">
              <Sparkles className="w-3 h-3 text-rose-600 fill-rose-600" />
              <span>Guest favourite</span>
            </span>
          )}
          <span className="px-2.5 py-1 bg-slate-900/80 backdrop-blur-md rounded-full text-[11px] font-medium text-white shadow-sm flex items-center gap-1">
      ...
      <div className="pt-3 pb-1 flex flex-col">
        ...
        <div className="flex items-baseline justify-between gap-2">
          ...
          <span className="shrink-0 flex items-center gap-1 text-xs font-semibold text-slate-900">
            ...
            <span>{ratingVal}</span>
            <span className="text-slate-400 text-[11px] font-normal">({reviewsCount})</span>
          </span>
        </div>
